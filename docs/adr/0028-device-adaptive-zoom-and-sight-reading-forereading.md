...
To maintain clean stepped increments, this value is rounded to the nearest $5\%$ ($0.05$) increment:
$$Z_{\text{stepped}} = \frac{\lfloor 20 \times Z_{\text{fit}} + 0.5 \rfloor}{20}$$
...
- **iPhone 13 Mini in Portrait ($W = 375\text{ px}$)**:
  - 4/4 meter with 8th notes ($W_{\text{measure}} = 520\text{ px}$): $Z = 375 / (520 + 135) = 0.572 \to \mathbf{55\%}$ (or $60\%$), allowing the whole measure to be seen at once.
  - 4/4 meter with 16th notes ($W_{\text{measure}} = 880\text{ px}$): $Z = 375 / (880 + 135) = 0.369 \to \mathbf{35\%}$.
...
- **iPad Portrait ($W = 768\text{ px}$)**:
  ...
  - 4/4 meter with 16th notes: $Z = 768 / 1015 = 0.756 \to \mathbf{75\%}$.

### 2. Expanded Dynamic Range (`MIN_ZOOM = 0.3`)
`MIN_ZOOM` was lowered from `0.5` to `0.3` (30%). On modern High-DPI mobile devices (e.g. Retina `@3x`), rendering at $0.3\times$ to $0.35\times$ zoom yields $3 \times 0.35 = 1.05\times$ physical device pixel mapping, maintaining crisp notation glyphs without blurring or subpixel artifacts. The settings slider range was updated to `min="30" max="150" step="5"`.

### 3. State & Mode Separation (`auto` vs `manual`)
...
### 4. Zero-Drift & Memory Discipline Invariance
