...
3. **Standard Strong Copyleft (GPLv3)**:
   ...
   - However, for web applications, SaaS platforms, and cloud-hosted tools, traditional GPLv3 suffers from the **Application Service Provider (ASP) / SaaS loophole**: hosting a modified service on a server and granting users remote access over a network is legally not considered "distribution" or "conveyance" in many jurisdictions. A third party could fork Solfège Scroller, add substantial proprietary features, host it commercially online, and legally withhold their source code modifications from students and teachers.
...
The sole runtime dependency of Solfège Scroller is **VexFlow** (`vexflow: ^5.0.0`).
...
- Added a permanent, clean link in the web user interface footer (`shortcuts-hint` in `index.html`):
  ```html
  <span class="footer-source"><a href="https://github.com/Hand-Lock/solfege-scroller" target="_blank" rel="noopener noreferrer" title="View Source Code on GitHub (AGPL-3.0-or-later)">AGPL-3.0 Source</a></span>
  ```
...
- **Closure of SaaS Loophole**: Prevents edtech companies from turning Solfège Scroller into a closed-source SaaS product without contributing improvements back to the community.
...
### Considerations
- Commercial organizations that refuse to use AGPL software internally or externally will not be able to embed Solfège Scroller into proprietary closed-source applications without releasing their code under AGPLv3. This is the desired reciprocal behavior of strict copyleft.
